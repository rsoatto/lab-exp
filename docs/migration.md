...
git add .lab-exp.toml <pkg>/INDEX.md experiments/_templates && git commit   # the framework files are part of the project history
```
`init` gitignores `experiments/_registry.tsv` (+ `.lock`): the TSV is a cache of README
front-matter, regenerated by `lab-exp registry --rebuild`. Contributors who don't use lab-exp
never see it.
...
## Stop tracking the registry in an existing repo

Projects adopted before the registry became derivable have the TSV in git. To make lab-exp
optional for other contributors:

```bash
lab-exp registry --sync              # one-time: cached facts -> README front-matter (additive)
git rm --cached experiments/_registry.tsv
printf 'experiments/_registry.tsv\nexperiments/_registry.tsv.lock\n' >> .gitignore
git add .gitignore experiments/*/README.md && git commit -m "lab-exp: registry is derived from READMEs"
lab-exp registry --rebuild           # everyone: regenerate the local cache after pulling
```
Front-matter is the shared contract; a collaborator's hand-made experiment shows up in `list` and
`dag` as soon as its README has one (with `command:` if it launches its own way).
...
- out/ artifacts with no `run-meta.json` → someone bypassed `lab-exp run` (a `note`, not an issue, for experiments lab-exp never created)
